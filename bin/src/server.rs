...
pub async fn get_qmdl(
    ...
    let qmdl_file = qmdl_store
        ...
        .await
        .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, format!("error opening QMDL file: {}", err)))?;
...
pub async fn restart_daemon(
    ...
    if let Some(sender) = restart_tx.take() {
        sender
            .send(())
            .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "couldn't send restart signal".to_string()))?;
...
pub async fn get_config(
    ...
    let config = parse_config(&state.config_path)
        .await
        .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, format!("failed to read config file: {}", err)))?;
...
pub async fn set_config(
    ...
    let config_str = toml::to_string_pretty(&config)
        .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, format!("failed to serialize config as TOML: {}", err)))?;

    write(&state.config_path, config_str)
        .await
        .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, format!("failed to write config file: {}", err)))?;
