...
pub fn run_diag_read_thread(
    ...
    mut gps_rx: Receiver<GpsCoordinate>,
    ...
    task_tracker.spawn(async move {
        ...
        let mut maybe_gps_writer: Option<GpsWriter> = None;
        ...
        // Create initial GPS writer
        {
            let qmdl_store = qmdl_store_lock.read().await;
            if let Some((_, entry)) = qmdl_store.get_current_entry() {
                match GpsWriter::new(entry.name.clone(), qmdl_store.path.to_str().unwrap()).await {
                    Ok(gps_writer) => {
                        maybe_gps_writer = Some(gps_writer);
                        info!("GPS writer created for scan: {}", entry.name);
                    }
                    Err(e) => {
                        warn!("Failed to create GPS writer: {}", e);
                    }
                }
            }
        }
        loop {
            tokio::select! {
                msg = qmdl_file_rx.recv() => {
                    match msg {
                        Some(DiagDeviceCtrlMessage::StartRecording) => {
                            ...
                            // Close previous GPS writer
                            if let Some(gps_writer) = maybe_gps_writer.take() {
                                if let Err(e) = gps_writer.close().await {
                                    warn!("Failed to close GPS writer: {}", e);
                                }
                            }

                            // Create new GPS writer for the new scan
                            if let Some((_, entry)) = qmdl_store.get_current_entry() {
                                match GpsWriter::new(entry.name.clone(), qmdl_store.path.to_str().unwrap()).await {
                                    Ok(gps_writer) => {
                                        maybe_gps_writer = Some(gps_writer);
                                        info!("GPS writer created for new scan: {}", entry.name);
                                    }
                                    Err(e) => {
                                        warn!("Failed to create GPS writer for new scan: {}", e);
                                    }
                                }
                            }
                        ...
                        Some(DiagDeviceCtrlMessage::StopRecording) => {
                            ...
                            if let Some(gps_writer) = maybe_gps_writer.take() {
                                if let Err(e) = gps_writer.close().await {
                                    ...
                                    info!("GPS writer closed successfully");
                                }
                            }
                        ...
                        Some(DiagDeviceCtrlMessage::Exit) | None => {
                            ...
                            // Close GPS writer on exit
                            if let Some(gps_writer) = maybe_gps_writer.take() {
                                if let Err(e) = gps_writer.close().await {
                                    warn!("Failed to close GPS writer on exit: {}", e);
                                }
                            }
                ...
                gps_coord = gps_rx.recv() => {
                    if let Some(coord) = gps_coord {
                        if let Some(gps_writer) = maybe_gps_writer.as_mut() {
                            if let Err(e) = gps_writer.write_coordinate(&coord).await {
                                error!("Failed to write GPS coordinate: {}", e);
                            } else {
                                debug!("GPS coordinate written: lat={}, lon={}", coord.latitude, coord.longitude);
                            }
                        } else {
                            warn!("Received GPS coordinate but no GPS writer is active");
                        }
                    }
                }
