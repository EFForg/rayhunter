...
impl DiagDevice {
    ...
    pub fn as_stream(
        ...
        futures::stream::try_unfold(self, |dev| async {
            ...
            /*
            match dev.get_next_messages_container().await {
                Ok(container) => {
                    return Ok(Some((container, dev)));
                }
                Err(_) => {
                    return Ok(None);
                }
    ...
    async fn get_next_messages_container(&mut self) -> Result<MessagesContainer, DiagDeviceError> {
        ...
        //let ((leftover_bytes, _), container) =
        //    MessagesContainer::from_bytes((&self.read_buf[0..bytes_read], 0))
        ...
        /*
        info!("MessageContainer = {:?}", container);

        if !leftover_bytes.is_empty() {
            warn!(
                "warning: {} leftover bytes when parsing MessagesContainer",
                leftover_bytes.len(),
            );
        }
        Ok(container)
    ...
    async fn retrieve_id_ranges(&mut self) -> DiagResult<[u32; 16]> {
        ...
        for msg in self.read_response().await? {
            match msg {
                Ok(Message::Log { .. }) => info!("skipping log response...{:?}", msg),
                Ok(Message::Response {
                    payload, status, ..
                }) => match payload {
    ...
    pub async fn config_logs(&mut self) -> DiagResult<()> {
        ...
        for (log_type, &log_mask_bitsize) in log_mask_sizes.iter().enumerate() {
            if log_mask_bitsize > 0 {
                info!(
                    "enabling logging for log type {}:{}",
                    log_type, log_mask_bitsize
                );
                self.set_log_mask(log_type as u32, log_mask_bitsize).await?;
                info!(
                    "enabled logging for log type {}:{}",
                    log_type, log_mask_bitsize
                );
            }
        }
