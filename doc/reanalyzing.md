...
**Capture formats and unreadable inputs:** both the legacy pcap format
(tcpdump's default output) and pcapng are accepted; the format is detected
from the file's magic number, not its extension. A file that can't be
opened or parsed is skipped with an error on stderr and doesn't stop the
run — every other input is still analyzed and reported. If a file stops
being readable partway through (e.g. a truncated recording), the rows
analyzed up to that point are kept. The exit code is non-zero when any
input couldn't be processed, so scripts can detect incomplete runs.
