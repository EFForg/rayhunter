...
The UZ801's WCN36xx (PRONTO) radio supports concurrent AP+STA mode. Rayhunter can connect to an existing WiFi network while keeping the USB hotspot running. See [WiFi Client Mode](./configuration.md#wifi-client-mode) for setup.

The interface creation method differs from the Orbic (which uses `iw`): the UZ801 creates a P2P_CLIENT virtual interface via nl80211 and converts it to a managed STATION interface. This is handled automatically by the daemon.
