...
Interface creation goes through nl80211 on every device: Rayhunter first asks the driver for a managed STATION interface and, if the driver refuses (as the Orbic's QCA6174 does), falls back to creating a P2P_CLIENT virtual interface and converting it to STATION. This is handled by the daemon when the feature is enabled.
