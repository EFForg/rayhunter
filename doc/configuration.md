...
- **Device UI Level**, which defines what Rayhunter shows on device's built-in screen. *Device UI Level* could be:
  ...
  - *Severity indicator images (Orbic only)*: shows a severity-specific PNG *and* colored line. The default PNGs are bundled from `daemon/images/orbic/severity/` when building from source. After installation, you can upload replacement PNGs through the web UI, which stores them in `/data/rayhunter/orbic-display-images/`. The filenames are `indicator_default.png`, `indicator_low.png`, `indicator_medium.png`, and `indicator_high.png`. Uploaded PNGs are stored on the device and used for the matching severity slot. You can reset any uploaded slot in the web UI to fall back to the bundled default PNG again.
