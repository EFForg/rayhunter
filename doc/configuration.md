...
- **Keep WiFi Always On** (TMOHS1 and CT2MHS01 only) disables the Qualcomm WLAN auto-shutdown timer. The timer normally turns off the WiFi access point after 10 min with no connected clients. When the setting is enabled, the device's wifi stays on indefinitely so you can always reach the web UI remotely. This uses more battery. A device reboot is required after changing this setting. See the [TMOHS1](./tmobile-tmohs1.md#wifi-auto-shutdown) and [CT2MHS01](./wingtech-ct2mhs01.md#wifi-auto-shutdown) device pages for details.
