...
struct Args {
    ...
    #[arg(
        short = 'o',
        long,
        help = "Optional directory for output files. With --format json, NDJSON is also written to <output>/<input>.ndjson alongside stdout. With --pcapify, PCAPNG files go here instead of next to each .qmdl."
    )]
...
#[cfg(test)]
mod tests {
    use super::{output_path, should_emit_to_ndjson, sidecar_output_dir};
